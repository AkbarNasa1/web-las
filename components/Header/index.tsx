...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // Fallback: kalau user sudah scroll sampai dasar halaman,
    // paksa section terakhir di menuData jadi aktif
    ...
    handleScrollBottom(); // cek langsung saat mount, jaga-jaga halaman pendek
  ...
  // Helper: tentukan apakah menu item lagi aktif
  const isMenuActive = (menuItem: Menu) => {
    const path = menuItem.path ?? "";
    const isHashLink = path.includes("/#");

    if (isHashLink) {
      const sectionId = path.split("#")[1];
      return pathUrl === "/" && activeSection === sectionId;
    }

    if (path === "/") {
      return pathUrl === "/" && activeSection === "";
    }
